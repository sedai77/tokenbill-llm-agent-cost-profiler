...
- **ingest_paths:** options are completed from the Env (name key, org key as principal key for
  ...
  whole when one adapter claims its files; an unrecognized file is a `UsageError`; the naive usage is
