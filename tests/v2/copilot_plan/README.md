...
| `test_edges.py` | pricer errors, unpriced fast cells, December month grain, linked levers with nothing to act on, wrong-kind seat findings, org names outside the grammar, org-mode entity resolution, other months and legacy rows, `None` sequences, helpers; privacy: no `p_` pseudonym, canary login or team label in any plan text |
| `test_perf.py` | addendum §17: 50k monthly cells, 6 levers ≤ 10 s CPU (`perf` marker); PR variant 5k cells ≤ 2 s (best of three; skipped under coverage) |
