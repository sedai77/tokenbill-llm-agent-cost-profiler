...
| `test_publication.py` | publication through `core.kanon`: every scope maps to a `requests` count filter (`channel github_copilot`, team, lane kind, model, workload class, agent product); team-scoped findings (incl. the `aggregate` kinds) re-scope on the Copilot chain keeping `product` / `billing_class` (R-E16); cohorts at k publish unchanged |
