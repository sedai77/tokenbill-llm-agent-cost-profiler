...
- **Categories** follow the brief (step 7: "categories `aggregate`"): every finding is category
  `aggregate` except the `dq.skipped-kinds` note (`data-quality`); the addendum's per-kind categories
  of §10.0 (premium / lever / failure / attribution) are design background. Category never exempts a
  `product`-scoped finding (R-E16): k-anonymity follows `COUNT_SOURCE`.
- **Scope** is `product: copilot`, `entity`, then `team` — or `cost_center` for report rows without
  a team (brief step 7 "`team` / `cost_center`"), neither for unattributed usage — plus `model`
  (cache-health, forced-migration) and `plan_scenario`. Never `editor_family` (CORE-AMENDMENTS
  CA-46: not a scope dim); editor shares are evidence attrs. Activity kinds are scoped by team.
