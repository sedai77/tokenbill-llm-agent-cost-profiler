...
- **Fixes:** a fix already targeting `github-copilot` is kept and a None fix stays None (the catalog
  is not consulted); a `fix_for` answer that is not a `github-copilot` `Fix` counts as "none"; the
  stripped fix keeps text and `doc_url`, drops `config_patch`, `target` and the applicability
  `gates` (they qualify the dropped Claude Code patch); exceptions from `fix_for` propagate.
...
- **`min_usd_gate`:** True iff the compared point ≥ `min_usd_nano(ctx)`: the recoverable point
  (Copilot scopes: max of the priced recoverable and headroom points); with neither priced, the
  `cost_observed` point; an unpriced value never passes (R2), as DETECT-CACHE's own gate does.
