...
| `test_cache_rules.py` | every Appendix A.13 row, both D28 branches in detail (numeric dotted versions, unknown/garbage versions, normalized model ids, channels), every table row (Anthropic channels, `openai_api` 5.6+ TTL, Azure scope `subscription`), fallback rows, caching |
...
| `test_shapley.py` | Appendix A.7 (8 / 18 / 4, Σ = 30), order independence, one evaluation per coalition, k > 10 raises, efficiency with v(∅) ≠ 0, negative values, brute-force agreement (hypothesis), MC within 3 SE of exact on a 5-player game, MC determinism, `scale_credits` exact totals (hand case + property), `largest_remainder` |
