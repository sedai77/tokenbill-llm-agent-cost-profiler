...
| `test_gate_ext.py` | **gate** (`@pytest.mark.gate`, skipped until F-KIT-C is merged): `count_users_fn` with the real `scope_counter` / `COUNT_SOURCE`; `run_reconcilers` on the real `MemoryStore` once it implements `LedgerStats` |
