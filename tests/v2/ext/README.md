...
| `test_gate_ext.py` | **gate** (`@pytest.mark.gate`, skipped until F-KIT-C is merged): `count_users_fn` with the real `scope_counter` / `COUNT_SOURCE`; `run_reconcilers` on the real `MemoryStore` once it implements `LedgerStats`; `persist` + `capabilities_present` on the real `MemoryRecordStore` (the activity-report-only handoff); all three passed on a throw-away overlay with F-KIT-C's committed `kanon` / `catalog` / `testing` (pkg/F-KIT-C @ 24513e8, 2026-09-24; never committed here) |
...
- `notes` (`list[DataQualityNote] | None = None`) can be passed by keyword on every function that
  can resolve a module (brief item 1); where CA-39 lists it positionally (`extension_rate_files`,
  `persist`, `showback`) it may also be passed by position, so callers written against either text
  work. `persist`, `retain`, `purge` and `capabilities_present` accept it for a uniform signature.
  `persist` takes no `accepted_key_ids` (withdrawn by CA-47 / R-E21).
- `capabilities_present` finds ledger requests on extension channels with one
  `store.aggregate(group_by=("channel",))`: SPEC §7.2 whitelists `channel` for SqliteStore's
  `GROUP BY`, whereas `iter_requests`' `where` is specified over lane / request columns only and
  `channel` is an inference column (so a `where={"channel": …}` probe could raise or be ignored).
