...
| `test_enterprise.py` | usage/cost/user_usage/user_cost endpoints; `amount`/`list_amount` exact; provisional vs final around the 30-day window; team rollups and suppression; whole directory; wrapper hint on empty pages |
| `test_openai_admin.py` | usage buckets (disjoint buckets, 30-min write TTL, derived uncached, sum checks, tiers, user rows summed, unix-second buckets); costs exact from JSON numbers, currency, API-key folding; FakePricer prices the buckets |
...
pair (`recon_pairs`). RECON's and F-KIT's gate tests read it. The content canary is planted in
